CREATE TABLE Accesoes (
    `Id` nvarchar(128) NOT NULL,
    `Credencial` nvarchar(128),
    `Fecha` int NOT NULL,
    `Hora` int NOT NULL,
    CONSTRAINT Accesoes PRIMARY KEY (`Id`)
)
CREATE TABLE Credencials (
    `Id` nvarchar(128) NOT NULL,
    `PasswordHash` nvarchar(65534),
    `Intentos` int NOT NULL,
    `FechaDesbloqueo` int NOT NULL,
    `Usuario_Id` nvarchar(128),
    CONSTRAINT Credencials PRIMARY KEY (`Id`)
)
CREATE INDEX IX_Usuario_Id ON Credencials(`Usuario_Id`)
CREATE TABLE Usuarios (
    `Id` nvarchar(128) NOT NULL,
    `Email` nvarchar(200),
    `Nombre` nvarchar(200),
    `Apellidos` nvarchar(200),
    `FechaAlta` int NOT NULL,
    `FechaNacimiento` int NOT NULL,
    CONSTRAINT Usuarios PRIMARY KEY (`Id`)
)
CREATE TABLE Rols (
    `Id` nvarchar(128) NOT NULL,
    `Nombre` nvarchar(200),
    CONSTRAINT Rols PRIMARY KEY (`Id`)
)
CREATE TABLE RolUsuarios (
    `Rol_Id` nvarchar(128) NOT NULL,
    `Usuario_Id` nvarchar(128) NOT NULL,
    CONSTRAINT RolUsuarios PRIMARY KEY (`Rol_Id`, `Usuario_Id`)
)
CREATE INDEX IX_Rol_Id ON RolUsuarios(`Rol_Id`)
CREATE INDEX IX_Usuario_Id ON RolUsuarios(`Usuario_Id`)
ALTER TABLE Credencials ADD CONSTRAINT FK_Credencials_Usuarios_Usuario_Id FOREIGN KEY (`Usuario_Id`) REFERENCES Usuarios (`Id`)
ALTER TABLE RolUsuarios ADD CONSTRAINT FK_RolUsuarios_Rols_Rol_Id FOREIGN KEY (`Rol_Id`) REFERENCES Rols (`Id`) ON DELETE CASCADE
ALTER TABLE RolUsuarios ADD CONSTRAINT FK_RolUsuarios_Usuarios_Usuario_Id FOREIGN KEY (`Usuario_Id`) REFERENCES Usuarios (`Id`) ON DELETE CASCADE
CREATE TABLE __MigrationHistory (
        `MigrationId` nvarchar(150) NOT NULL,
        `ContextKey` nvarchar(300) NOT NULL,
        `Model` varbinary(60000) NOT NULL,
    `ProductVersion` nvarchar(32) NOT NULL,
    CONSTRAINT __MigrationHistory PRIMARY KEY (`MigrationId`, `ContextKey`)
)
INSERT __MigrationHistory(`MigrationId`, `ContextKey`, `Model`, `ProductVersion`)
VALUES (N'201508142047118_Inicial', N'ED.Datos.Migrations.Configuration',   , N'6.1.3-40302')